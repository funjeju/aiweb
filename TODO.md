...
### 별자리 에셋 수익화 (향후)
- [ ] **에셋 유무료 분리** — 현재 전체 무료. 희귀/프리미엄 에셋(나만의 우주선, 계절 한정 캐릭터 등)을 유료 전환.
- [ ] **에셋 상점** — `/store` 페이지: 에셋 카탈로그, 크레딧으로 구매, 구매 내역 저장.
- [ ] **나만의 에셋 업로드** — 유저가 직접 이미지/GIF 업로드 → 심사 후 개인 에셋으로 등록 (유료 기능).
- [ ] **에셋 패키지** — 테마별 묶음 판매 (예: 할로윈 우주, 크리스마스 우주).
- [ ] **크리에이터 에셋 마켓** — 크리에이터가 직접 에셋 등록 → 수익 배분.
